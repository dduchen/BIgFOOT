# note - WiP - change file creation/etc to account for gene_id argument
# - temporary fix - just create a temporary directory for each gene and perform inference there
...
def main():
    ...
    parser.add_argument('gene_id', type=str, help='gene_identifier')
...
def write_trimmed_contig_info(graph, paths,gene_id):
    """Reconstruct contig sequences from their node paths and write to fasta."""
    contig_seqs = {}
    fasta_file_name = "trimmed_contigs_{}.fasta".format(gene_id)
    paths_file_name = "trimmed_contigs_{}.paths".format(gene_id)
    with open(fasta_file_name, 'w') as f:
        with open(paths_file_name, 'w') as f2:
