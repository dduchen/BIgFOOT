...
def main():
    ...
    parser.add_argument('--wfmash_param', dest='wfmash_param', type=float, required=True, help='Segment length for wfmash-based overlaps estimation')
    ...
    parser.add_argument('-o', '--out_fasta', dest='fasta', type=str, default='haps_long.final.fasta', help='Output fasta file with final haplotypes')
    ...
    global wfmash_segment
    wfmash_segment = float(args.wfmash_param)
    ...
    if args.toboggan:
        ...
        # Run toboggan
        toboggan_output = "toboggan_long.out"
...
def contig_paths_to_nodes(cg_paths, g, paths, start_info, low_ab_nodes,
    ...
    with open("haps_long.fasta", 'w') as f_haps:
...
def write_toboggan_graph(flow_graph, x, outfile='toboggan_long.graph'):
...
def build_genome_graph(graph, paths, final_ab):
    ...
    graph.save("genome_graph_long.gt")
    write_genome_gfa(graph, final_paths, old_to_new_nodes, "genome_graph_long.gfa")
    return
...
def write_trimmed_contig_info(graph, paths):
    ...
    contig_seqs = {}
    with open("trimmed_contigs_long.fasta", 'w') as f:
        with open("trimmed_contigs_long.paths", 'w') as f2:
...
def write_intermediate_files(graph, paths, adj_out, start_info, contig_ab=None):
    ...
    with open("trimmed_contigs_long.gfa", 'w') as f:
        f.write("H\tVN:Z:1.0\n")
        for c, adj_list in adj_out.items():
...
def build_adj_info(graph, paths, nvert, low_ab_nodes, trim_size, included_paths):
    ...
    print("Note: skipping much of this part - 'removing inclusions due to trimming/extension'")
...
def get_adj_out(paths, g, low_ab_nodes, del_set):
    ...
    minimap2_overlaps = "overlaps_long.minimap2.paf"
    input = "trimmed_contigs_long.fasta"
    subprocess.check_call(
        "samtools faidx {0}; wfmash -t 32 -p 95 -c {4} -l {3} -s {2} -X {0} {0} > {1}".format(
            input, minimap2_overlaps, wfmash_segment, wfmash_segment*2.5, wfmash_segment*5),
        shell=True
    )
