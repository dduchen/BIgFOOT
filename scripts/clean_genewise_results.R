library(data.table)
currdir=getwd()
sample_id=gsub("/familywise.*$","",currdir);
...
if(as.numeric(grepl("_pe",currdir))==1){
    aln_type="pe"
...
#
depth_file=list.files(path="../",pattern="depth_raw.txt")
graph_results_file<-list.files(path="../",pattern="results_raw.txt")
depth<-fread(paste0("../",depth_file),header=T)
graph_results<-fread(paste0("../",graph_results_file),header=F)
sample_id<-colnames(depth)[1]
#
