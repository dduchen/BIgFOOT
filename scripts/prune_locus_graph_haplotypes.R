...
target_haps<-target_haps[target_haps$offtarget_hits<=0.25,]
target_haps$num_offtarget<-length(nodes_for_filtering)
offtarget_haps<-offtarget_haps[offtarget_haps$offtarget_hits>=0.75,]
offtarget_haps$num_offtarget<-length(nodes_for_filtering)
#
###############################################################################################################
fwrite(data.frame(target_haps),file=gsub("exact.txt","exact_ids.txt",hap_exact_file),quote=F,sep="\t",col.names=F,row.names=F)
fwrite(data.frame(offtarget_haps),file=gsub("offtarget.txt","offtarget_ids.txt",hap_offtarget_file),quote=F,sep="\t",col.names=F,row.names=F)
###############################################################################################################